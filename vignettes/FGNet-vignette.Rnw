...
The report functions are wrappers that include several steps. We will now see each of them:
	...
	\item Create the incidence matrices to build the network
...
\subsection{Step 3: Create the incidence matrices to build the network}
\label{sec:Step3}
...
<<eval=TRUE>>=
incidMatFiltered <- toMatrix(results, threshold=0.2,
...
To build the networks based on terms, use the argument \emph{key="Terms"}. See section \emph{\ref{sec:TermsNw} Terms network} for further details.

\subsection{Step 4: Build and plot the networks}
\label{sec:Step4}

\subsubsection{Functional network}
Once the incidence matrices are ready, they can be transformed into the network. The function \emph{functionalNetwork()} generates and plots the network:
<<fig=TRUE, eval=TRUE>>=
...
\subsubsection{Intersection network}
In addition to the functional network, it is also possible to draw a the \emph{intersection network} if there are nodes in several metagroups.
This plot is a simplified functional network, containing only the nodes in several metagroups and the metagroups they belong to. 'Metagroup nodes' are coloured, and can be seen as a cluster of all the genes/proteins that belong only to that metagroup. 
By default, the intersection network is automatically plotted when requesting an interactive plot, but it can also be plotted by itself:
...
To do so, all we need to do is to build the incidence matrices based on terms instead of genes using the argument \emph{key="Terms"}.
<<eval=TRUE>>=
results <- getResults_gtLinker(jobID=1639610, jobName="Alzheimer")
...
intersectionNetwork(incidMatTerms, plotType="static", plotTitle="Terms in several metagroups")
@

Since GeneTerm Linker filters out generic and redundant terms from the final metagroups, by default these terms are not plotted. To include them in the graph, set the argument \emph{removeFiltered=FALSE}.
<<eval=TRUE, fig=TRUE>>=
incidMatTerms <- toMatrix(results, key="Terms", removeFiltered=FALSE)
intersectionNetwork(incidMatTerms, plotType="static", 
...
\textbf{A) Generating the report automatically} (getting an initial overview of the networks):\\
...
<<eval=TRUE, echo=FALSE>>=
txtFile <- paste(file.path(system.file('examples', package='FGNet')), "David_Metabolism.txt", sep="/")
...
<<eval=TRUE, fig=TRUE, width=9>>=
distMat <- plotMetagroupsDistance(incidMat)
@
...
@
<<eval=TRUE, results=hide>>=
incidMatFiltered <- toMatrix(results, 
...
incidMatSelectedGroups <- toMatrix(resultsSclusters, 
...
Another way to explore the results from the analysis is the terms network. However, since DAVID's gene-term sets only include one term, the default functional network will have no edges. Instead, plot the network based on the clusters (stablishig a link between terms in the same cluster):
...
functionalNetwork(incidMatTerms$clustersMatrix, plotType="dynamic",
 weighted=TRUE, eColor="grey")
intersectionNetwork(incidMatTerms$clustersMatrix, plotType="static", 
