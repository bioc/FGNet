...
incidMatFiltered <- fea2incidMat(feaAlzheimer, 
    filterAttribute="Silhouette Width", filterOperator="<", filterThreshold=0)
@
...
The default plot will plot all the genes/terms in the network, and will return the netwoks as igraph objects and matrices in an invisible list:
<<fig=TRUE, eval=TRUE>>=
...
incidMatTerms <- fea2incidMat(feaAlzheimer$metagroups, clusterColumn="Metagroup", 
    key="Terms",
    filterAttribute="Silhouette.Width", filterThreshold=0)
...
par(mfrow=c(1,2))
functionalNetwork(incidMatTerms, 
...
\subsection{Filtering and selecting clusters}
\label{sec:filtering}
In this section we will analyze with DAVID a list with genes from 2 metabolic pathways in yeast: \emph{ergosterol biosynthesis} and \emph{sphingolipid metabolism}, as an example of a vnetwork with very overlapping clusters.
...
\subsubsection{Filtering based on a \textbf{cluster propperty}}
...
i.e. Selecting the clusters with highest Enrichment Score or least genes:
...
\subsubsection{Selecting \textbf{clusters with specific terms or keywords}}
...
\subsubsection{Selecting \textbf{specific groups}}
...
functionalNetwork(incidMatSelection)
@
...
<<eval=TRUE, fig=TRUE, width=10>>=
incidMat_alzh <- fea2incidMat(feaAlzheimer)
stats <- analyzeNetwork(incidMat_alzh)
@
<<eval=TRUE>>=
names(stats)
stats$intraHubsCount
@
