...
\item Personalize or analyze a specific network (sec.\ref{sec:fea2incidMat})
\end{enumerate}
...
geneList1 <- setNames(c(rep(1,10),rep(-1,5)), geneList1)
...
Here we will show a simple example with each of the tools:
...
DAVID and topGO work in a very symilar way: Just provide a gene list and set the arguments:
<<eval=TRUE>>=
library(FGNet)
geneList1 <- c("YBL084C", "YDL008W", "YDR118W", "YDR301W", "YDR448W", 
    "YFR036W", "YGL240W", "YHR166C", "YKL022C", "YLR102C", "YLR115W", 
...
@ 
\textbf{DAVID}:\\
Requires internet connection (and registration at at \url{http://david.abcc.ncifcrf.gov/webservice/register.htm}).
<<eval=FALSE>>=
results_David <- fea_david(geneList1, geneLabels=geneLabels, 
    email="example@email.com")
@ 
\textbf{TopGO}:\\
TopGO relies on local databases. Therefore, it does not require internet connection. Note that the the results from topGO are provided as individual gene-term sets, not grouped into clusters:
<<eval=FALSE>>=
...
Gene-Term Linker is also run over a gene list. Since the analysis usually takes several minutes to be ready, the workflow is divided in two steps: sending the analysis request, and retrieving the results:
<<eval=TRUE>>=
...
\subsubsection{Personalizing networks}
